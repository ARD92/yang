...
To Do:
1. BFD update APIs not utilized. A particular session can only added or deleted. Cannot be modified.
2. complete using the fileOperations function to save session ids in Json in case the application dies and restarts.
The dictionary in memory would be cleared. Currently commented.
'''
import grpc
import os
import sys
...
def sessionReq(**kwargs):
    ...
    print(laddr, raddr, intf, ri, timer) 
...
def find(xml):
   mapp = {}
   xml = bs(xml, 'lxml')
   if xml.echolite.get("nc:operation"):
       mapp["root"] = xml.echolite.get("nc:operation")
       logging.info("Operation {} occured at top level".format(xml.echolite.get("nc:operation")))
       return mapp
   else:
       print("entering else")
       total = []
       ...
       for session in sessions:
           print(session.get("nc:operation"))
           if session.get("nc:operation"):
               mapp = {} # added this which is not clean
               logging.info("Operation {} occured at sessions level ".format(session.get("nc:operation")))
               mapp["session:{}".format(session.findChild('name').text)] = session.get("nc:operation")
               total.append(mapp) 
       logging.info(total) 
       return(total)


def fileOperations(action):
    if action == 'read':
        with open('bfd_session_store.json', 'r') as f:
            fdata = json.load(f.read())
        if fdata:
            BFD_SESSION_STORE = data
    elif action == 'write':
        with open('bfd_session_store.json','w') as f:
            f.write(json.dumps(BFD_SESSION_STORE))
        ...
        if isinstance(out, dict) and out["root"] == 'create':
            ...
            for session in xml.echolite.findAll('session'):
                ...
                try:
                    ...
                    if status.code == 0:
                        print("success")
                        BFD_SESSION_STORE[param['session_name']]=sid
                except:
                    print("Received Exception 1")
                    logging.error("Session delete failed. Exception received")
            channel.close()
            ...
            print(" Root delete")
            try:
                for name,sid in BFD_SESSION_STORE.items():
                    status = sessionDel(bfd,sid)
                    print(status)
                    if status.code == 0:
                        BFD_SESSION_STORE.pop(name)
            except:
                print("Exception block 2")
                logging.error("Session delete failed 2. Exception received")
            channel.close()
            ...
            for ses in out:
                ...
                if oper == 'delete':
                    ...
                    if name in BFD_SESSION_STORE.keys():
                        sid = BFD_SESSION_STORE[name]
                        print(sid)
                        result = sessionDel(bfd,sid)
                        print(result)
                        if result.code == 0:
                            BFD_SESSION_STORE.pop(name)
                            logging.info("Session {} deleted".format(name))
                        else:
                            logging.error("Session {} not deleted".format(name))
                    ...
                    if status.code == 0:
                        ...
                        logging.info("Session added with id {}".format(sid))
                    else:
                        logging.error(status, sid)
                        print(status, sid)
            channel.close()
...
def run():
    ...
    #try:
    #    if not BFD_SESSION_STORE:
    #        fileOperations(read)
    #except:
    #    f2 = open('bfd_session_store.json','a+')
    #    f2.close()
    #    pass
    client.loop_forever()
